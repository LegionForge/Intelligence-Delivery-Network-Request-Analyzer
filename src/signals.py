...
# Minimum complexity floor per domain — ensures domain-aware tier promotion
DOMAIN_COMPLEXITY_FLOOR = {
    "legal":    0.55,   # always L3 once legal domain is detected
    "health":   0.40,   # L2 floor
    "finance":  0.46,   # L2 floor
    "research": 0.46,   # L2 floor
    "agentic":  0.46,   # L2 floor
    "coding":   0.28,   # L1 floor
    "creative": 0.20,
    "general":  0.00,
}

def _word_in(word: str, text: str) -> bool:
    """Whole-word match to avoid false substring hits (e.g. 'api' in 'capital')."""
...
def _python_analyze(prompt: str, offline: bool = False) -> AnalysisResult:
    ...
    # FIX 1: All domain keywords use whole-word matching via _word_in
    DOMAIN_KW = {
        "coding":   [("refactor",0.9),("function",0.6),("class",0.6),("bug",0.8),
                     ...
                     ("fault-tolerant",1.0),("microservice",0.9),("latency",0.8),("scalability",0.9),("replication",0.9),("semantics",0.7),("event",0.6),("streaming",0.8),("unit test",0.8)],
        "research": [("analyze",0.8),("literature",1.0),("hypothesis",1.0),("dataset",0.9),
                     ...
                     ("transformer",0.8),("efficiency",0.6),("peer-reviewed",1.0),("architecture",0.7),("summarize",0.7)],
        ...
        "finance":  [("portfolio",1.0),("revenue",0.9),("roi",1.0),("balance sheet",1.0),
                     ...
                     ("forecast",0.8),("q3",0.7),("fy2026",0.9)],
        ...
        "agentic":  [("automate",1.0),("workflow",1.0),("webhook",1.0),("cron",1.0),
                     ...
                     ("slack",0.8),("alert",0.7),("every hour",0.9)],
    ...
    for domain, kws in DOMAIN_KW.items():
        # FIX 1: use whole-word matching for all single-word keywords
        score = 0.0
        for kw, w in kws:
            if ' ' in kw:
                if kw in lower:
    ...
    # FIX 2: Normalise token contribution against short-prompt baseline (200 tokens)
    # so that a 10-word prompt still registers meaningful complexity.
    token_norm = min(token_est / 200, 1.0)

    # FIX 3: Add domain-intrinsic complexity boost
    domain_score_val = domain_scores.get(primary_domain, 0)
    domain_boost = clamp(domain_score_val / 3.0, 0.0, 0.35)
    ...
    # FIX 4: Apply domain complexity floor so high-domain prompts always get correct tier
    # For coding, use a tiered floor: L1 for weak matches, L2 for strong matches
    floor = DOMAIN_COMPLEXITY_FLOOR.get(primary_domain, 0.0)
    if primary_domain == "coding" and domain_score_val > 1.2:
        floor = 0.46   # strong coding match → L2
    if primary_domain == "coding" and domain_score_val > 2.5:
        floor = 0.55   # very high complexity coding → ensures complexity > 0.50
    ...
    if any(_word_in(k, lower) for k in ["patient","diagnosis","prescription","medication","blood pressure","glucose","lab result"]):
        pii.append("medical")
